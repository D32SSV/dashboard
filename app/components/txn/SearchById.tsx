...
const SearchById = ({ token }: { token: string | null }) => {
  ...
  return (
    <section className="px-2 pt-1 pb-2">
      <div className="w-full flex items-center justify-center flex-col gap-4">
        <div className="w-max px-2">
          ...
          <form
            ...
            className="max-w-[75vw] md:max-w-[38vw] mb-8"
            aria-labelledby="txn-form"
          >
            <label
              htmlFor="txnId"
              className="block text-sm font-medium inverseTheme mt-2 mb-1"
            >
              Transaction ID
            </label>
      ...
      {txnData && (
        <article className="px-8 py-4 rounded-md shadow-sm inverseTheme max-h-max break-words border lg:max-w-2/3">
          ...
          <dl className="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4 max-[325px]:grid-cols-1 max-[325px]:gap-x-28 ">
            {Object.entries(txnDetailsToShow(txnData)).map(([label, value]) => (
              <div key={label}>
                <dt className="font-normal text-lg text-gray-300">{label}</dt>
